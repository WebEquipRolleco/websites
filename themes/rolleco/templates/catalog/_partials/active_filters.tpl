...
<section id="js-active-search-filters" class="{if $activeFilters|count}active_filters{else}hide{/if}">
  {block name='active_filters_title'}
   <span> class="h6 {if $activeFilters|count}active-filter-title{else}hidden-xs-up{/if}">{l s='Active filters' d='Shop.Theme.Global'}</span>
  {/block}
  ...
  {if $activeFilters|count}
    <ul>
      {foreach from=$activeFilters item="filter"}
        ...
        {/block}
      {/foreach}
      <li class="filter-block bg-danger">
        <a href="#" data-search-url="{$clear_all_link}" class=" js-search-filters-clear-all">
