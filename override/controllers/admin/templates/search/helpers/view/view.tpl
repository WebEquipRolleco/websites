...
{if $query && isset($nb_results) && $nb_results}
    ...
    {if isset($after_sales) && $after_sales}
        <div class="panel">
            <h3>{l s='%d SAV' sprintf=[$after_sales|@count] d='Admin.Navigation.Search'} </h3>
            <table class="table">
                <tr>
                    <th><b>{l s="Référence"}</b></th>
                    <th class="text-center"><b>{l s="Client"}</b></th>
                    <th class="text-center"><b>{l s="Statut"}</b></th>
                    <th class="text-center"><b>{l s="Création"}</b></th>
                    <th></th>
                </tr>
                {foreach from=$after_sales item=sav}
                    <tr>
                        <td>{$sav->reference}</td>
                        <td class="text-center">{$sav->getCustomer()->firstname} {$sav->getCustomer()->lastname}</td>
                        <td class="text-center"><span class='label label-{$sav->getStatusClass()}'>{$sav->getStatusLabel()}</span></td>
                        <td class="text-center">{$sav->date_add|date_format:'d/m/Y'}</td>
                        <td class="text-right">
                            <a href="{$link->getAdminLink('AdminAfterSales')}&updateafter_sale&id_after_sale={$sav->id}" class="btn btn-default">
                                <i class="icon-search-plus"></i> {l s='Afficher'}
                            </a>
                        </td>
                    </tr>
                {/foreach}
